...
/***/ function(module, exports, __webpack_require__) {
	...
	__webpack_require__(/*! ../css/introduce.css */ 27);
	...
	$('#header').html(header);
	$('#footer').html(footer);
	__webpack_require__(/*! ./headerNav.js */ 29); //先载入header模板，base才能操作其中dom
	
	/* ------------------------------------------------------------
	 * 图片滚动
	 * ------------------------------------------------------------ */
	$('.inforSlider').each(function () {
	    $(this).inforSlider(3);
	});
	 ...
	 * 章程切换
	 * ------------------------------------------------------------ */
	var consitList = $('.constituList').children();
	var consitConentList = $('.constituContent').children();
	consitList.each(function () {
	    var $this = $(this);
	    var index = $this.attr('index');
	    $this.click(function () {
	        consitList.filter('.active').removeClass('active');
	        $this.addClass('active');
	        consitConentList.filter('.block').removeClass('block').addClass('none');
	        consitConentList.eq(index).removeClass('none').addClass('block');
	    });
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	/* WEBPACK VAR INJECTION */(function($) {(function () {
	  ...
	  */
		...
		secendNavList.each(function () {
			...
			$this.click(function () {
				secendNavunderLine.stop();
				secendNavunderLine.animate({ left: index * 150 }, 300);
				secendNavList.filter('.active').removeClass('active');
				$this.addClass('active');
				secendContent.filter('.block').removeClass('block').addClass('none');
				secendContent.eq(index).removeClass('none').addClass('block');
			});
		});
	  ...
	  * 
	  */
		var currentNavTag = locationHash.slice(-1) || 0;
		secendNavList.eq(currentNavTag).click();
	
		BarSecend.eq(currentNav).children().children().each(function () {
			$(this).click(function () {
				var index = $(this).attr('index');
				secendNavList.eq(index).click();
			});
		});
	})();
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(/*! jquery */ 1)))
...
//# sourceMappingURL=introduce.bundle.js.map